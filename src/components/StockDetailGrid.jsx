...
const MODE_4_MINUTE = '4minute'; // 4支股票分时图模式，每个网格显示一支股票的分时图
...
const StockDetailGrid = () => {
  ...
  // 从 URL 参数中获取初始模式和 tagDate
  const urlParams = new URLSearchParams(location.search);
  const initialMode = urlParams.get('mode') || MODE_1_STOCK;
  const initialTagDate = urlParams.get('tagDate') || null;
  const initialIndex = parseInt(urlParams.get('index') || '0', 10) || 0;
  
  // 模式选择：1支、2支、4支、4分时
  const [mode, setMode] = useState(initialMode);
  
  // 4分时模式下使用的日期（从 URL 参数 tagDate 获取）
  const [minuteViewDate, setMinuteViewDate] = useState(initialTagDate);
  
  // 4分时模式下每支股票的分时数据 { stockCode: minuteData[] }
  const [minuteDataMap, setMinuteDataMap] = useState({});
  ...
  // 获取4分时模式下多支股票的分时数据（每支股票使用各自的日期）
  const fetchMinuteDataBatch = useCallback(async (stockDatePairs) => {
    if (!stockDatePairs || stockDatePairs.length === 0) return;
    
    try {
      const response = await post(`${API_HOST}/stock/getStockOneMinuteData`, stockDatePairs);
      
      if (response && typeof response === 'object') {
        // 直接替换，不合并旧数据
        setMinuteDataMap(response);
      }
    } catch (error) {
      console.error('批量获取分时数据失败:', error);
    }
  }, []);

  // MODE_4_MINUTE 模式下，加载当前显示的4支股票的分时数据
  useEffect(() => {
    if (mode !== MODE_4_MINUTE) return;
    
    const currentStocks = stockList.slice(baseIndex, baseIndex + GRID_COUNT_4);
    // 每支股票使用各自的 tagDate
    const stockDatePairs = currentStocks
      .filter(s => s?.stockCode && s?.tagDate)
      .map(s => ({ stockCode: s.stockCode, date: s.tagDate }));
    
    if (stockDatePairs.length > 0) {
      fetchMinuteDataBatch(stockDatePairs);
    }
  }, [mode, baseIndex, stockList, fetchMinuteDataBatch]);
  ...
  useEffect(() => {
    ...
    if (mode === MODE_1_STOCK) {
      ...
      // 4分时模式不需要加载K线数据，只加载详情
      const currentStocks = stockList.slice(baseIndex, baseIndex + GRID_COUNT_4);
      const minuteStockCodes = currentStocks
        .filter(stock => stock?.stockCode)
        .map(stock => stock.stockCode);
      
      if (minuteStockCodes.length > 0) {
        fetchStockDetailBatch(minuteStockCodes);
        minuteStockCodes.forEach(stockCode => {
          fetchLatestStockData(stockCode);
        });
      }
      return; // 直接返回，不执行后面的 K线数据加载
    }
  ...
  // 渲染所有网格的图表
  useEffect(() => {
    // MODE_4_MINUTE 模式下，图表由单独的 useEffect 处理，跳过
    if (mode === MODE_4_MINUTE) {
      return;
    }
  ...
  useEffect(() => {
    ...
    // 检查当前显示的股票是否有分时数据
    const hasAnyData = currentStocks.some(stock => 
      stock?.stockCode && minuteDataMap[stock.stockCode] && minuteDataMap[stock.stockCode].length > 0
    );
    
    // 如果没有任何数据，不清理旧图表（等待数据加载）
    if (!hasAnyData) return;
    
    // 清理旧的4分时图实例
    Object.keys(chartsRef.current).forEach(chartKey => {
      if (chartKey.includes('-minute-grid-')) {
        const chartGroup = chartsRef.current[chartKey];
        if (chartGroup && chartGroup.cleanup) {
          chartGroup.cleanup();
        }
    ...
    const renderTimer = setTimeout(() => {
      currentStocks.forEach((stock, index) => {
        ...
        const stockMinuteData = minuteDataMap[stock.stockCode];
        if (!stockMinuteData || stockMinuteData.length === 0) return;
        
        const chartDomId = `minute-grid-chart-${index}`;
        const chartDom = document.getElementById(chartDomId);
        if (!chartDom) return;
        
        const chartKey = `${stock.stockCode}-minute-grid-${index}`;
  ...
  }, [mode, baseIndex, minuteDataMap, stockList]);
  ...
  const renderGridContent = () => {
    if (mode === MODE_1_STOCK) {
      ...
      return currentStocks.map((stock, index) => {
        const stockCode = stock?.stockCode;
        const stockMinuteData = stockCode ? minuteDataMap[stockCode] : null;
        const hasData = stockMinuteData && stockMinuteData.length > 0;
        const gridKey = `${stockCode || 'empty'}-minute-${index}`;
        const isSelected = selectedGrid === gridKey;
        ...
        return (
          ...
          >
            <GridHeader
              title={`${stock?.stockName || '--'} (${stockCode || '--'}) ${stock?.tagDate || ''}`}
              stockCode={stockCode}
              isSelected={isSelected}
              gridKey={gridKey}
              onDoubleClick={() => {
            ...
            <div style={{ flex: '100%', position: 'relative' }}>
              ...
              <div id={`minute-grid-chart-${index}`} style={{ width: '100%', height: '100%' }}></div>
            </div>
          </div>
        );
      });
    ...
    <div 
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1, justifyContent: 'space-between' }}>
          ...
          {mode === MODE_4_MINUTE && (
            <>
              <span style={{ color: '#fff', fontSize: '12px' }}>分时图模式</span>
              <span style={{ marginLeft: '20px', color: '#fff', fontSize: '12px' }}>
                {baseIndex + 1}-{Math.min(baseIndex + GRID_COUNT_4, stockList.length)} / {stockList.length}
              </span>
            </>
          ...
          <button
            onClick={() => setMode(MODE_4_MINUTE)}
            style={{
              padding: '4px 6px',
              background: mode === MODE_4_MINUTE ? '#1e90ff' : '#23263a',
              border: mode === MODE_4_MINUTE ? '2px solid #1e90ff' : '1px solid #444',
              borderRadius: '4px',
              cursor: 'pointer',
              outline: 'none',
              transition: 'all 0.2s',
              ...
              justifyContent: 'center',
              height: '28px',
              color: mode === MODE_4_MINUTE ? '#fff' : '#888',
              fontSize: '11px',
              fontWeight: mode === MODE_4_MINUTE ? 'bold' : 'normal',
            }}
            onMouseOver={e => {
              if (mode !== MODE_4_MINUTE) {
                e.currentTarget.style.background = '#2a2f3a';
