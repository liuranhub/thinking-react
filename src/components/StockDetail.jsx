...
const HIGHLIGHT_TAG_CONFIG = [
  ...
  { tagName: '电力', color: '#1e90ff' },    // 亮蓝
  { tagName: '电网', color: '#1e90ff' },    // 亮蓝
  { tagName: '风能', color: '#1e90ff' },    // 亮蓝
...
const mottoTags = ['不破不立', '等待', '今日长缨在手,何时缚住苍龙',
   '任何时候,机会只在少数人','如何100%拍死蚊子,等它趴下' , '大盘涨时你不涨,大盘跌时你不跌'];
...
const StockDetail = () => {
  ...
  useEffect(() => {
    ...
    } else if (currentStock.stockName && currentStock.stockCode) {
      // 如果没有传递标题，使用股票名称和代码作为默认标题
      document.title = `${currentStock.stockName} (${currentStock.stockCode})`;
    }
  ...
  const handleCalculateStockData = useCallback(async () => {
    ...
    try {
      ...
      const data = await post(`${browserHostServer}/stock/stockScoreAnalyser/${currentStockCode}/${currentDate}`);
        ...
        client.subscribe('/topic/broadcast', (message) => {
          ...
          if (data.type === 'loadDetail' && data.stockCode) {
            ...
            if (targetStockCode !== currentStockCode) {
              ...
              // 当前股票与目标股票相同，调用 stockScoreAnalyser 刷新分数
              const endDate = chartEndDateRef.current;
              if (endDate) {
                console.log('收到刷新请求，调用 stockScoreAnalyser:', currentStockCode, endDate);
                post(API_HOST + `/stock/stockScoreAnalyser/${currentStockCode}/${endDate}`)
                  .then(result => {
                    if (result) {
                      setApiScoreResult(result);
                      setChartRefreshTrigger(Date.now());
                    }
                  })
                  .catch(err => console.error('stockScoreAnalyser 调用失败:', err));
              }
            }
          }
        });
...
const getWarmUpStockCodes = () => {
  ...
  const renderCharts = () => {
    ...
    const maList = selectedMAs && selectedMAs.length > 0 ? MA_CONFIG.filter(ma => selectedMAs.includes(ma.key)).map(ma => ({
      ...ma,
      data: calcMA(ma.key)
    })) : [];
      ...
      tooltip: {
        ...
        formatter: function (params) {
          ...
          if (currentIndex >= 0) {
            ...
            if (endIdx > startIdx && currentVolume > 0) {
              // 计算前60个交易日的平均交易量
              let totalVolume = 0;
              let count = 0;
              
              for (let i = startIdx; i < endIdx; i++) {
                const volume = chartData[i]?.chenJiaoLiang ?? 0;
                if (volume > 0) {
                  totalVolume += volume;
                  count++;
                }
              ...
              if (count > 0) {
                ...
                liangBi2 = ratio.toFixed(1);
              }
            }
          }

          // 计算量比Max：当前K线交易量 / 前10根K线的最大交易量
          // let liangBiMax = '--';
          // if (currentIndex >= 0) {
          //   const currentVolume = currentData?.chenJiaoLiang ?? 0;
          ...
          //     }
          //     if (maxVolume > 0) {
          //       liangBiMax = (currentVolume / maxVolume).toFixed(2);
          //     }
          //   }
          // }
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={
            ...
            <div style={{display: 'flex', flexWrap: 'wrap', gap: '8px'}}>
              <span style={{color: TEXT_COLOR}}>
                波动系数(月): {apiScoreResult.scoreDetail?.volatilityScoreMonthConfig?.value}
              </span>
            </div>
            <div style={{display: 'flex', flexWrap: 'wrap', gap: '8px'}}>
              <span style={{color: TEXT_COLOR}}>
                波动系数(年): {apiScoreResult.scoreDetail?.volatilityScoreYearConfig?.value}
              </span>
            </div>
          </div>
                          ...
                          {apiScoreResult.scoreDetail && Object.keys(apiScoreResult.scoreDetail)
                            .filter(key => {
                              ...
                              return item?.type === 'main' && 
                                     key !== 'volatilityScoreMonthConfig' && 
                                     key !== 'volatilityScoreYearConfig';
                            })
                            .map(key => {
                              const item = apiScoreResult.scoreDetail[key];
        ...
        {/* 网格视图按钮组 */}
            ...
            <Select
              value={selectedMAs}
              onChange={value => setSelectedMAs(value || [])}
              mode="multiple"
              style={{ width: 100, height: 25, background: '#181c26', color: '#fff', border: 'none', overflow: 'hidden' }}
